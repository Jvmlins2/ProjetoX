...
const char *mqtt_id = "esp32-senai134-magicduelcontrole2";
...
float temperatura = 0;
float umidade = 0;
float setupUmidade = 37.0;
float setupTemperatura = 22.5;
...
void ataqueFogo ()
{
  if (temperatura> (setupTemperatura* 1.09))
  {
    forcaAtaqueFogo = 2;
  }
  else if (temperatura >= (setupTemperatura*1.02) && temperatura <= (setupTemperatura*1.09) )
  {
    forcaAtaqueFogo = 1;
  }
  else 
...
void enviaForcaAtaque()
{
  ...
  doc["forcaAtaqueFogo"] = forcaAtaqueFogo;
  doc["forcaAtaqueGelo"] = forcaAtaqueGelo;
  doc["sensorPronto2"] = sensorPronto;
